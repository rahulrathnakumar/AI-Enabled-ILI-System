...
def measure(img_name, seg, var, x, y, z):
    ...
    def __length__(cnt):
        ...
        # cv2.drawContours(rectImg,[box],0,255,-1)
        # defectIndices = np.where(rectImg == 255)
        # xList_defect = x[defectIndices]
        # yList_defect = y[defectIndices]
        # zList_defect = z[defectIndices]
        # pointsList = np.transpose(np.asarray([xList_defect, yList_defect, zList_defect]))
        # return scipy.spatial.distance.pdist(X = pointsList, metric = 'euclidean').max()
